...
async function generateLevel(fromLang, toLang, level) {
  ...
  const count = POOL_STRUCTURE.street.cardsPerLevel
  const prompt = `Generate exactly ${count} street language flashcards (Level ${level}, ${spec.diff}) for a ${fromName} speaker learning ${toName}.
Focus: ${spec.focus}
...
export default async function handler(req, res) {
  ...
  const level = Math.min(POOL_STRUCTURE.street.totalLevels, Math.max(1, body.level || 1))
