...
[{"front":"...","back":"...","pronunciation":"...","wordType":"noun"}]`
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      system: 'You are a professional language educator. Generate accurate vocabulary flashcards. Return ONLY valid JSON array, no markdown fences.\nQUALITY RULES: All words must be natural, correct language as used by native speakers. Never invent words or use incorrect forms. Double-check every translation for accuracy. Verbs in English always with "to": "to read", "to write", "to eat". Never write verbs without the "to" prefix. Each card must be unique — no duplicate translations within one level.\nCRITICAL: The example sentence for each card MUST contain the target word being learned. Never show an example sentence that does not include the word on the card.\nSWAHILI PHONETICS: For Swahili words, pronunciation must follow Swahili phonetic rules — every letter is pronounced, stress on second-to-last syllable. Use simple syllable notation: "ku-SO-ma" for "kusoma". Never use German or English phonetic notation for Swahili words.',
