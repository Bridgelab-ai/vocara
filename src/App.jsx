...
async function saveSessionState(uid, queue, index, newProgress) {
  try {
    await setDoc(doc(db, 'users', uid, 'session', 'current'), { queue, index, newProgress, savedAt: Date.now() })
...
const T = {
  de: {
    ...
    myProgress: 'Dein Fortschritt', partnerLearning: 'Partner lernt', notActive: 'Noch kein Partner',
  ...
  en: {
    ...
    myProgress: 'Your progress', partnerLearning: 'Partner is learning', notActive: 'No partner yet',
...
function PlacementTest({ lang, theme, user, onBack, onSaveCefr }) {
  ...
  if (done) {
    ...
    return (
      <div style={s.container}><div style={s.homeBox}>
...
function MenuScreen({ user, myData, partnerData, allCards, lang, onSaveProgress, theme, onThemeChange, onPartnerUpdate, onSaveCefr }) {
  ...
  if (screen === 'cards' && session) return (
    <CardScreen session={session} onBack={() => setScreen('menu')} onFinish={handleFinish}
      lang={lang} cardProgress={cardProgress} s={s} onSaveState={handleSaveState}
      startIndex={resumeStartIndex} startProgress={resumeStartProgress} />
  )
...
function App() {
  ...
  return (
    <MenuScreen user={user} myData={myData} partnerData={partnerData}
